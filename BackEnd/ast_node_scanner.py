...
def package_node_scanner(string):
    # 给定包节点名称，在图中查找该节点是否已经建立
    package_node = graph.nodes.match("Package").where("_.name=" + "'" + string[14:] + "'").first()
    if package_node is None:  # 若没有则创建新节点
        package_node = Node("Package", name=string[14:])
        graph.create(package_node)
...
class my_scanner:
    ...
    def scan_driver(self):
        ...
        while self.location < self.num_of_lines:
            ...
            if string.find("Import") != -1:
                package_node_scanner(string)
            self.location = self.location + 1
        self.location = 0
        # 第二轮，建立边
        while self.location < self.num_of_lines:
            string = get_line(self.filename, self.location)
            self.package_relation_scanner(string, "Empty")
    ...
    def package_relation_scanner(self, string, node):
        if string.find("ImportFrom") != -1:
            self.scan_importfrom(string)
        if string.find("Import") != -1:  # 发现包之间的引用关系
    ...
    def scan_importfrom(self, string):
        self.location = self.location + 1
        str_update = "Go"
        # 确认该类节点是否建立，在neo库中进行搜索
        importfrom_node = graph.nodes.match("Package").where("_.name=" + "'" + string[14:] + "'").first()
        if importfrom_node is None:  # 若该包在数据库中没有节点则创建新节点
            importfrom_node = Node("Package", name=string[14:])
        while (str_update.find("EndPackage") == -1) & (self.location < self.num_of_lines):
            str_input = get_line(self.filename, self.location)
            str_update = str_input
            self.package_relation_scanner(str_input, importfrom_node)
        # 由于已经到end行，回退一行
        self.location = self.location - 1

    def scan_import(self, node):
        import_Name = get_line(self.filename, self.location)
        while import_Name.find("Name :") == -1:
            self.location = self.location + 1
            import_Name = get_line(self.filename, self.location)
        if node == "Empty":
            return None
        # 确认该包节点是否建立
        import_node = graph.nodes.match("Package").where("_.name=" + "'" + import_Name[8:] + "'").first()
        if import_node is not None:
            entity = Relationship(node, "imports", import_node)
            graph.create(entity)
